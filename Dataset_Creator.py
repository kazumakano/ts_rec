...
def _save_ts_fig_img(cam_name: str, img: np.ndarray, label: int, usr_name: str, vid_date: date) -> None:
    ...
    cv2.imwrite(path.join(TGT_ROOT_DIR, file_name), img)
...
def _undo(cam_name: str, vid_date: date) -> None:
    ...
    file = glob(path.join(TGT_ROOT_DIR, f"*_{vid_date.isoformat()}_{cam_name}_{st.session_state['vid_idx']:02d}_{st.session_state['digit']}_[0-9].tif"))[0]
    os.remove(file)

    st.info(f"deleted {path.basename(file)}")

def _undo_btn(cam_name: str, vid_date: date) -> None:
    st.button("undo", on_click=lambda: _undo(cam_name, vid_date), disabled=st.session_state["vid_idx"] == 0 and st.session_state["digit"] == 0)

def _skip_btn() -> None:
    st.button("skip", on_click=_next_state)
...
def home() -> None:
    ...
    if usr_name != "":
        ...
        if cam_dir is not None:
            ...
            if len(files) == 0:
                st.write("all images are labeled")
            else:
                file = files[0]
                _write_img_desc(file)

                img = util.extract_ts_fig(util.read_head_n_frms(file, 1).squeeze())[st.session_state["digit"]]
