...
def _try(datamodule: DataModule, param: dict) -> None:    ######
    ...
    trainer.fit(CNN(param, torch.Tensor([len(datamodule.dataset["train"]) / v for v in datamodule.dataset["train"].breakdown.values()])), datamodule=datamodule)

def tune_params(param_list_file: str, ts_fig_dir: str, bot_conf_file: Optional[str] = None, result_dir_name: Optional[str] = None) -> None:
